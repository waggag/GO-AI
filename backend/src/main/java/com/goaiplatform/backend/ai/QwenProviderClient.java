...
public class QwenProviderClient implements AiProviderClient {
    ...
    public QwenProviderClient(
            ...
            @Value("${spring.ai.qwen.api-key:}") String fallbackApiKey
    ) {
    ...
    public Flux<String> stream(Long userId, String model, String prompt, Double temperature, Double topP, Integer maxTokens, List<String> context) {
        ...
        return resolveApiKey(userId)
                ...
                .onErrorResume(ex -> Flux.just("通义千问调用失败：" + ex.getMessage()));
    }

    private Mono<String> resolveApiKey(Long userId) {
        return apiKeyManagementService.resolveProviderKey(userId, ChatDtos.Provider.QWEN)
                .switchIfEmpty(Mono.justOrEmpty(fallbackApiKey))
                .switchIfEmpty(Mono.error(new IllegalStateException("通义千问未配置API Key")));
    }
