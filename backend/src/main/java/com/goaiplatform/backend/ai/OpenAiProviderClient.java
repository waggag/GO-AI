...
public class OpenAiProviderClient implements AiProviderClient {
    ...
    public ChatDtos.Provider provider() {
        return ChatDtos.Provider.OPENAI;
    }
    ...
    public Flux<String> stream(Long userId, String model, String prompt, Double temperature, Double topP, Integer maxTokens, List<String> context) {
        ...
        return resolveApiKey(userId)
                ...
                .onErrorResume(ex -> Flux.just("OpenAI调用失败：" + ex.getMessage()));
    }

    private Mono<String> resolveApiKey(Long userId) {
        return apiKeyManagementService.resolveProviderKey(userId, ChatDtos.Provider.OPENAI)
                .switchIfEmpty(Mono.justOrEmpty(fallbackApiKey))
                .switchIfEmpty(Mono.error(new IllegalStateException("OPENAI未配置API Key")));
    }
